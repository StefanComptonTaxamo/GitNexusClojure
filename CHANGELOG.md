...
- **Clojure language support (experimental)** — first-class indexing for `.clj`/`.cljc`/`.cljs`/`.edn` via vendored `tree-sitter-clojure`. Captures `ns`, `defn`/`defn-`/`definline`, `def` (with `(def x (fn …))` / `(def x #(…))` promoted to Function), `defprotocol`, `defrecord`/`deftype`, `definterface`, `defmulti`, `defmethod`, `:require`/`:use`/`:import`. Resolves Clojure imports across files (hyphen → underscore translation, `.clj`/`.cljc`/`.cljs` lookup). Emits `CALLS` edges for plain function calls, with special forms, threading-macro heads, and `clojure.core` builtins filtered; namespace-qualified calls (`str/upper-case`, `Math/abs`) and Java-interop dot-prefix forms (`.toString`) preserve their qualifier in the call name. Models multimethods: `defmulti` is a Function with `isMultimethod=true`; each `defmethod` is a distinct Method node carrying its `dispatchValue`; a new `DISPATCHES_TO` graph relationship links each method back to its multimethod. Emits `IMPLEMENTS` edges for both inline `(defrecord Name [fields] Proto …)` and free-floating `(extend-protocol Proto Type …)`/`(extend-type Type Proto …)`/`(extend Type Proto …)` forms — heritage edges resolve cross-file via the symbol table even when the form lives in a file that contains neither the type nor the protocol (new `extractForm` channel on `HeritageExtractor`). Adds the `clojure-protocol` MroStrategy and `dialect`/`isMultimethod`/`dispatchValue` node properties. Threading-macro call expansion, `:refer` bindings, reader-conditional dialect tagging, and the dedicated scope resolver are landing in follow-up PRs.
