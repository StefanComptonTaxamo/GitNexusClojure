...
¹ Clojure support is **experimental**. The current cut produces Module / Function / Method / Class / Trait / Interface / Variable nodes for the core forms (`ns`, `defn`/`defn-`/`definline`, `def` (including `(def x (fn …))` / `(def x #(…))` promotion to Function), `defprotocol`, `defrecord`/`deftype`, `definterface`, `defmulti`, `defmethod`); resolves `:require`/`:use`/`:import` clauses to `IMPORTS` edges; emits `CALLS` edges for plain function calls (special forms, threading-macro heads, and `clojure.core` builtins are filtered); and models multimethods with `isMultimethod=true` on the `defmulti` Function plus `DISPATCHES_TO` edges from each `defmethod` Method (carrying its `dispatchValue`) back to the multimethod. Retroactive heritage (`extend-protocol`/`extend-type`/`extend`), `:refer` named bindings, threading-macro call expansion, reader-conditional dialect tagging, and the dedicated scope resolver are landing in follow-up PRs.
